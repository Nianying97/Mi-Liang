...
class GCN_LSTM(nn.Module):
    def __init__(self, input_dim, lstm_dim, gcn_hidden, num_classes, dropout=0.5):
        super().__init__()
        self.linear_gcn = nn.Linear(input_dim, gcn_hidden)
        self.lstm = nn.LSTM(gcn_hidden * 16, lstm_dim, batch_first=True, num_layers=2)
    ...
    def forward(self, x):
        ...
        B, T, N, D = x.shape
        gcn_outputs = []
        for t in range(T):    # 一层GCN就行
            x_t = x[:, t, :, :]  # [B, N, F]
            norm_x = F.normalize(x_t, dim=-1)
            A = torch.matmul(norm_x, norm_x.transpose(1, 2))  # [B, N, N]
            A = F.softmax(A, dim=-1)

            XW1 = self.linear_gcn(x_t)  # [B, N, gcn_hidden1]
            AXW1 = torch.matmul(A, XW1)  # [B, N, gcn_hidden1]
            H = F.relu(self.dropout(AXW1))  # [B, N, gcn_hidden1]
            gcn_outputs.append(H)


        # 堆叠时间步输出： [B, T, N, gcn_hidden2]
        gcn_seq = torch.stack(gcn_outputs, dim=1)
        # reshape 为 LSTM 输入： [B, T, N * gcn_hidden2]
        gcn_seq = gcn_seq.view(B, T, -1)
        lstm_out, _ = self.lstm(gcn_seq)  # [B, T, lstm_hidden]
        out = lstm_out[:, -1, :]  # [B, lstm_hidden]
        return self.classifier(out)  # [B, output_dim]
...
model = GCN_LSTM(input_dim=26, lstm_dim=256, gcn_hidden=24, num_classes=7).to(device)
num_classes = 7
...
criterion = LDAMFocalLoss(cls_num_list=cls_num_list, max_m=0.3, s=30, gamma=2)
