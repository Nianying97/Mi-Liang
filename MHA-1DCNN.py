...
class MHACNN(nn.Module):
    def __init__(self, input_dim, num_heads, cnn_channels, num_classes):
        super(MHACNN, self).__init__()

        self.input_dim = input_dim  # 每节点的特征维度
        self.cnn_channels = cnn_channels

        # 多头注意力层：处理空间维度
        self.mha = nn.MultiheadAttention(embed_dim=input_dim, num_heads=num_heads, batch_first=True)

        # 1D-CNN 提取时间特征：沿 T 维度卷积
        self.cnn1 = nn.Conv1d(in_channels=input_dim, out_channels=cnn_channels, kernel_size=3, padding=1)
        self.cnn2 = nn.Conv1d(in_channels=cnn_channels, out_channels=cnn_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        # x: [B, T, N, F]
        B, T, N, D = x.shape
        x = x.reshape(B, T, N * D)  # → [B, T, N*F]
        # MHA 空间注意力（沿 N*F 特征维度）
        attn_out, _ = self.mha(x, x, x)  # [B, T, N*F]
        # 1D-CNN 处理时间序列（T维），所以转置 [B, C, T]
        x = attn_out.transpose(1, 2)  # [B, N*F, T]
        x = F.relu(self.cnn1(x))     # [B, cnn_channels, T]
        x = F.relu(self.cnn2(x))     # [B, 128, T]
        # 分类
...
model = MHACNN(input_dim=416, num_heads=4, cnn_channels=256, num_classes=7).to(device)
num_classes = 7
