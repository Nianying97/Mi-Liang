...
# 构建邻接矩阵 A
def batch_weighted_knn_graph(X, k=5, alpha=2.0):
    """
    输入:X: [B, N, F] - 每个样本一个图，共 B 个图，每个图 N 个节点，每个节点 F 个特征
    输出:A: [B, N, N] - 邻接权重矩阵
    """
    B, N, D = X.shape
    x_square = (X ** 2).sum(dim=2, keepdim=True)
    dist = x_square + x_square.transpose(1, 2) - 2 * torch.bmm(X, X.transpose(1, 2))  # [B, N, N]
    topk = dist.topk(k=k+1, dim=-1, largest=False)
    knn_indices = topk.indices[:, :, 1:]  # [B, N, k]
    A = torch.zeros(B, N, N, device=X.device)
    for i in range(k):
        idx = knn_indices[:, :, i]  # [B, N]
        b_idx = torch.arange(B).view(B, 1).expand(-1, N).to(X.device)
        n_idx = torch.arange(N).view(1, N).expand(B, -1).to(X.device)
        d = dist[b_idx, n_idx, idx]
        w = torch.exp(-alpha * d)
        A[b_idx, n_idx, idx] = w
...
# 高斯聚合
class GaussianGraphLayer(nn.Module):
    ...
    def forward(self, X, A):
        # A: [B, N, N], X: [B, N, F]
        H = torch.matmul(A, X)  # 图聚合
        mu = self.fc_mu(H)      # 高斯均值
        logvar = self.fc_logvar(H)
        std = torch.exp(0.5 * logvar)
        eps = torch.randn_like(std)
        z = mu + eps * std
        return z  # [B, N, latent_dim]
...
class SimpleGraphLayer(nn.Module):
    ...
    def forward(self, X, A):
        H = torch.matmul(A, X)  # [B, N, in_dim]
        H = self.linear(H)
        return H  # [B, N, out_dim]

class WKGNN(nn.Module):
    def __init__(self, input_dim, latent_dim, lstm_dim, num_classes, dropout=0.3):
        super().__init__()
        self.gaussian_gnn = SimpleGraphLayer(input_dim, latent_dim)
        self.lstm = nn.LSTM(latent_dim * 16, lstm_dim, batch_first=True, num_layers=2)  # 8个节点
    ...
    def forward(self, x):  # x: [B, T, N, F]
        ...
        for t in range(T):
            x_t = x[:, t, :, :]  # [B, N, F]
            A = batch_weighted_knn_graph(x_t, k=3, alpha=1.0)
            z_t = self.gaussian_gnn(x_t, A)  # [B, N, latent_dim] 高斯聚合
            z_seq.append(z_t)

        z_seq = torch.stack(z_seq, dim=1)  # [B, T, N, latent_dim]
        z_seq = z_seq.view(B, T, -1)  # [B, T, N * latent_dim]

        lstm_out, _ = self.lstm(z_seq)
        final_feat = lstm_out[:, -1, :]  # [B, lstm_dim]
        return self.classifier(self.dropout(final_feat))  # [B, num_classes]
...
model = WKGNN(input_dim=26, latent_dim=32, lstm_dim=256, num_classes=7).to(device)
num_classes = 7
